...
public class HologramCommand implements CommandExecutor, TabCompleter {
    ...
    private void registerSubcommands() {
        ...
        subcommands.add(new SwapPageCommand());
    ...
    private class SwapPageCommand extends Subcommand {
        public SwapPageCommand() {
            super("swappage", "交换两个页面的位置", "wooholograms.admin", Collections.emptyList());
        }

        @Override
        public boolean execute(CommandSender sender, String[] args) {
            if (args.length < 3) {
                sender.sendMessage(ColorUtil.colorize("&c用法: /wh swappage <名称> <页码1> <页码2>"));
                return true;
            }
