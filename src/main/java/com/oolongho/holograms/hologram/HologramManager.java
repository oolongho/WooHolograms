...
import com.oolongho.holograms.api.event.HologramDeleteEvent;
import com.oolongho.holograms.api.event.HologramsLoadedEvent;
...
public class HologramManager {
     ...
     * 重载所有全息图
     *
     * 注意：不保存内存状态到磁盘，直接销毁后从 yml 重新加载。
     * 对齐 DecentHolograms 和 FancyHolograms 的 reload 语义，
     * 保留用户对 yml 的手动编辑。
     */
    public void reload() {
        stopUpdateTask();
    ...
    public void onWorldLoad(String worldName) {
        // 加载因世界未加载而暂存的全息图
        if (storage instanceof com.oolongho.holograms.storage.YamlHologramStorage yamlStorage) {
